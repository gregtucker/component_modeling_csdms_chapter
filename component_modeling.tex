...
\todo[inline,color=yellow]{KRB: the section on landlab refers to this section below, so at some point it would be good to make sure its fully written state is not incompatible with that section}
...
%   a. Include table
% 3. Example of BMI use w/ figure (I like Greg's suggestion of Hoch paper.
...
%The result: trains can be assembled from combinations of all sorts of different rail cars,
%built by different companies, in different places, and with different purposes.
...
\todo{Reference JOSS paper in review.}
...
An example of using BMI with Fortran 2003
...
Show a code sample here.
PRMS (describe the model), written in Fortran.
Reference Regan paper.
Show and explain calls to initialize, update, finalize.
Comment how there's no need to know how PRMS works;
you just need to use the BMI functions to drive it (ha!).

An example of BMI use in current research.
Use a Hoch paper.
Show one of his figures.
