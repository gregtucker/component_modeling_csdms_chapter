...
One of the example is the National Water Model (NWM) data component. This data component can access and subset the forecasted streamflow time series generated by the NWM hydrologic modelling framework. Figure \label{fig:data_component1} shows how to use the data component to get the streamflow data at a river channel for a flooding event. Figure \label{fig:data_component2} shows the corresponding time series plot. This data component includes a set of standard control and query functions (e.g.,  initialize() , update()). These standard methods make the dataset easier to couple with numerical models that are implemented with BMI without needing to know the time series file format.
...
Landlab is a Python-language library designed to support the creation, combination, and re-use of 2-D models \cite{hobley2017creative, barnhart2020short}. 

A implementing has two core parts:
- the computational domain, the \textit{grid}. 
- one or more things that operate on the computational domain, each \textit{components}. 
If one has more than one component, one might want for them to be able to talk to one another. 

The former doesn't need to be different across applications. 

The design of Landlab provides three capabilities to support efficient creation of new models. 

First, the Landlab \textit{grid} is a data structure that represents the computational domain. Landlab provides multiple grid types through a similar interface (e.g., regular raster, network, regular hexagon, irregular). Each grid type provides a full set of adjacency information for the points, lines, and polygons that make up the computational mesh. Second, variables can be stored at any of the elements of the grid. These \textit{fields} provide a place to store model state variables. Third, Landlab provides a base class for a model \textit{component}, which enforces a standard. 


% What are the building blocks one might want to reduce friction in creating a new model. 
% - Grid (multiple types of grids, same interface)
% - Ability to store state variables. 
% - Template (python base class) for a process component. 
% Built in metadata about what state variables it needs (inputs) and modifies (outputs).

In addition we have a variety of plotting and misc numerical/gis-ish utilties. 

Testing, examples, and API documentation. 

B/c of grid+component metadata, almost all components are prepared to be pyMT components. If a component author provides the minimum metadata (which is required by the component base class), an init, and a r1s, then it is almost certainly pymt compatible. 

Using Landlab models... within pymt, write your own script, use something like  terrainbento\cite{barnhart2019terrainbento}.
