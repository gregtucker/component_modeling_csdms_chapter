...
Notes on model figure

T: flood or LS, glacier? andy n?, LEM

C: WLD?,  CEM?, RAFEM

M: turbins, Waipaoa?, sequence or sed flux
...
it allows us to only provide bridges from each supported language to the hub
...
frameworks and tools (e.g. The \textit{pymt},\textit{dakotathon}).

The Python Modeling Toolkit (\textit{pymt}; described in detail in Section~\ref{sec:pymt})
if a model-coupling framework that provides tools for using and coupling BMI-enabled components,
...
Operating as a BMI component, Landlab components operate as isolated elements that no longer
share a common grid and data, and requires an input file, which describes it's grid and data,
to initialize itself. This is by design and required by the BMI so that a user interacts
